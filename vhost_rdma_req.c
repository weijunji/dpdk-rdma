...
{
	switch (opcode) {
	case VIRTIO_RDMA_WR_RDMA_WRITE:
	...
	case VIRTIO_RDMA_WR_RDMA_WRITE_WITH_IMM:
		if (qp->req.opcode == IB_OPCODE_RC_RDMA_WRITE_FIRST ||
		    qp->req.opcode == IB_OPCODE_RC_RDMA_WRITE_MIDDLE)
			return fits ?
	...
	case VIRTIO_RDMA_WR_SEND_WITH_IMM:
		if (qp->req.opcode == IB_OPCODE_RC_SEND_FIRST ||
		    qp->req.opcode == IB_OPCODE_RC_SEND_MIDDLE)
			return fits ?
	...
	case VIRTIO_RDMA_WR_RDMA_READ:
		return IB_OPCODE_RC_RDMA_READ_REQUEST;

	case VIRTIO_RDMA_WR_ATOMIC_CMP_AND_SWP:
		return IB_OPCODE_RC_COMPARE_SWAP;

	case VIRTIO_RDMA_WR_ATOMIC_FETCH_AND_ADD:
		return IB_OPCODE_RC_FETCH_ADD;

	case VIRTIO_RDMA_WR_SEND_WITH_INV:
		if (qp->req.opcode == IB_OPCODE_RC_SEND_FIRST ||
		    qp->req.opcode == IB_OPCODE_RC_SEND_MIDDLE)
...
static int
vhost_rdma_do_local_ops(struct vhost_rdma_qp *qp,
						struct vhost_rdma_send_wqe *wqe)
{
	uint8_t opcode = wqe->wr->opcode;
	...
	case VIRTIO_RDMA_WR_LOCAL_INV:
		...
		//else
		ret = vhost_rdma_invalidate_mr(qp, rkey);
	...
	case VIRTIO_RDMA_WR_BIND_MW:
		...
		// break;
	default:
		RDMA_LOG_ERR_DP("Unexpected send wqe opcode %d\n", opcode);
		wqe->status = IB_WC_LOC_QP_OP_ERR;
		return -EINVAL;
	}
				...
				int opcode, int payload, struct vhost_rdma_pkt_info *pkt)
{
	...
	struct cmd_post_send *wr = wqe->wr;
	...
	qp_num = (pkt->mask & VHOST_DETH_MASK) ? wr->wr.ud.remote_qpn :
					 qp->attr.dest_qp_num;
	...
	if (pkt->mask & VHOST_IETH_MASK)
		ieth_set_rkey(pkt, wr->ex.invalidate_rkey);

	if (pkt->mask & VHOST_ATMETH_MASK) {
...
next_wqe:
	...
	if (wqe->mask & WR_LOCAL_OP_MASK) {
		ret = vhost_rdma_do_local_ops(qp, wqe);
		if (unlikely(ret))
			goto err;
		else
			goto next_wqe;
