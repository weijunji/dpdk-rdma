...
check_rkey(struct vhost_rdma_qp *qp, struct vhost_rdma_pkt_info *pkt)
{
	...
	if (pkt->mask & (VHOST_READ_MASK | VHOST_WRITE_MASK)) {
		...
		access = (pkt->mask & VHOST_READ_MASK) ? IB_ACCESS_REMOTE_READ
						     : IB_ACCESS_REMOTE_WRITE;
	} else if (pkt->mask & VHOST_ATOMIC_MASK) {
		qp->resp.va = atmeth_va(pkt);
		qp->resp.offset = 0;
		qp->resp.rkey = atmeth_rkey(pkt);
		qp->resp.resid = sizeof(uint64_t);
...
process_atomic(struct vhost_rdma_qp *qp, struct vhost_rdma_pkt_info *pkt)
{
	...
	vaddr = iova_to_vaddr(qp->dev->mem, mr, qp->resp.va + qp->resp.offset, sizeof(uint64_t));
	...
	uint8_t syndrome, uint32_t *crcp)
{
	...
	mbuf = vhost_rdma_init_packet(dev, &qp->pri_av, paylen, ack);
	if (!mbuf)
		return NULL;
...
static int invalidate_rkey(struct vhost_rdma_qp *qp, uint32_t rkey)
{
	// if (rkey_is_mw(rkey))
	// 	return rxe_invalidate_mw(qp, rkey);
	return vhost_rdma_invalidate_mr(qp, rkey);
}
...
do_complete(struct vhost_rdma_qp *qp, struct vhost_rdma_pkt_info *pkt)
{
	...
	if (cqe.status == IB_WC_SUCCESS) {
		 ...
		 * space
		 */
		cqe.wc_flags = IB_WC_GRH;
		if (pkt->mask & VHOST_IMMDT_MASK) {
			cqe.wc_flags |= IB_WC_WITH_IMM;
			cqe.ex.imm_data = immdt_imm(pkt);
		}

		if (pkt->mask & VHOST_IETH_MASK) {
			cqe.wc_flags |= IB_WC_WITH_INVALIDATE;
			cqe.ex.invalidate_rkey = ieth_rkey(pkt);
		}
