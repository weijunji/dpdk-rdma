...
wr_to_wc_opcode(enum virtio_rdma_wr_opcode opcode)
{
	switch (opcode) {
	case VIRTIO_RDMA_WR_RDMA_WRITE:			return IB_WC_RDMA_WRITE;
	case VIRTIO_RDMA_WR_RDMA_WRITE_WITH_IMM:		return IB_WC_RDMA_WRITE;
	case VIRTIO_RDMA_WR_SEND:			return IB_WC_SEND;
	case VIRTIO_RDMA_WR_SEND_WITH_IMM:		return IB_WC_SEND;
	case VIRTIO_RDMA_WR_RDMA_READ:			return IB_WC_RDMA_READ;
	case VIRTIO_RDMA_WR_ATOMIC_CMP_AND_SWP:		return IB_WC_COMP_SWAP;
	case VIRTIO_RDMA_WR_ATOMIC_FETCH_AND_ADD:	return IB_WC_FETCH_ADD;
	case VIRTIO_RDMA_WR_TSO:				return IB_WC_LSO;
	case VIRTIO_RDMA_WR_SEND_WITH_INV:		return IB_WC_SEND;
	case VIRTIO_RDMA_WR_RDMA_READ_WITH_INV:		return IB_WC_RDMA_READ;
	case VIRTIO_RDMA_WR_LOCAL_INV:			return IB_WC_LOCAL_INV;
	case VIRTIO_RDMA_WR_REG_MR:			return IB_WC_REG_MR;
	case VIRTIO_RDMA_WR_BIND_MW:			return IB_WC_BIND_MW;
			...
			struct vhost_rdma_send_wqe *wqe)
{
	...
	case IB_OPCODE_RC_RDMA_READ_RESPONSE_ONLY:
		 ...
		 */
	case IB_OPCODE_RC_RDMA_READ_RESPONSE_MIDDLE:
		if (wqe->wr->opcode != VIRTIO_RDMA_WR_RDMA_READ &&
		    wqe->wr->opcode != VIRTIO_RDMA_WR_RDMA_READ_WITH_INV) {
	...
	case IB_OPCODE_RC_ATOMIC_ACKNOWLEDGE:
		...
		if (wqe->wr->opcode != VIRTIO_RDMA_WR_ATOMIC_CMP_AND_SWP &&
		    wqe->wr->opcode != VIRTIO_RDMA_WR_ATOMIC_FETCH_AND_ADD)
			return COMPST_ERROR;
		reset_retry_counters(qp);
	...
	default:
		RDMA_LOG_ERR_DP("unexpected opcode\n");
	}
			  ...
			  struct virtio_rdma_cqe *cqe)
{
	...
	if (wqe->wr->opcode == VIRTIO_RDMA_WR_RDMA_WRITE_WITH_IMM ||
		wqe->wr->opcode == VIRTIO_RDMA_WR_SEND_WITH_IMM)
		cqe->wc_flags = IB_WC_WITH_IMM;
...
do_complete(struct vhost_rdma_qp *qp, struct vhost_rdma_send_wqe *wqe)
{
	...
	if (wqe->wr->opcode == VIRTIO_RDMA_WR_SEND ||
	    wqe->wr->opcode == VIRTIO_RDMA_WR_SEND_WITH_IMM ||
	    wqe->wr->opcode == VIRTIO_RDMA_WR_SEND_WITH_INV)
	...
	while (1) {
		...
		case COMPST_ERROR:
			if (wqe->status == IB_WC_SUCCESS)
				RDMA_LOG_ERR_DP("wqe->status == IB_WC_SUCCESS");
			do_complete(qp, wqe);
